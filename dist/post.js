...
var titleAppend = ' – David Matthams';
...
(0,petite_vue__WEBPACK_IMPORTED_MODULE_0__.createApp)({
      ...
      return _regeneratorRuntime().wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              fetch(apiUrl + "?query=*[slug.current == '" + getSlug() + "']").then(function (response) {
                return response.json();
              }).then(function (data) {
                if (data['result'].length) {
                  _this.post = data['result'][0];

                  _this.setMeta();

                  _this.getImage(_this.post.image);

                  _this.getDate(_this.post.publishedAt);
                } else {
                  window.location.replace('/404');
                }
  ...
  getImage: function getImage(ref) {
  ...
  getDate: function getDate(inputDate) {
